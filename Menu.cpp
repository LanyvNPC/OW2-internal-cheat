...
void Menu::Render() {
	...
	if (Config::Get().menuEnabled) {
		...
		bool _visible = true;
		if (ImGui::Begin(u8"                          MaxFlick  ", &_visible, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar))
		{
			if (!Config::Get().IsLogin)
			{
				...
				if (ImGui::Button(u8"Login", ImVec2(115, 25)))
				{
					...
					{
						string CodeVal = WebWinhttp("/Panel/login.php?mode=login&id=" + codeStr + "&COM=" + get_system_uuid());
						if (CodeVal.find(u8"0") != string::npos)
						{
			...
			else
			{
				...
				if (ImGui::Button(u8"저장", ImVec2(90, 20)))
				{
					SaveSetting(CurrentSettingItem_English);
				}

				ImGui::SameLine();
				if (ImGui::Button(u8"불러오기", ImVec2(90, 20)))
				{
				...
				ImGui::Text(u8"월핵류");
				ImGui::Checkbox((u8"테두리 월핵"), &Config::Get().UseGlowESP);
				ImGui::Checkbox((u8"체력"), &Config::Get().HealthESP);
				ImGui::Separator();

				ImGui::Text(u8"에임류");
				ImGui::Checkbox((u8"부드러운 에임봇"), &Config::Get().TAimbot);
				ImGui::Checkbox((u8"부드러운 끌어치기 에임봇"), &Config::Get().FAimbot);
				ImGui::Checkbox((u8"부드러운 예측 에임봇"), &Config::Get().TPAimbot);
				ImGui::Checkbox((u8"부드러운 한조 예측 에임봇"), &Config::Get().HanzoAimbot);
			    ImGui::Checkbox((u8"자동 발사"), &Config::Get().TriggerBot);
				ImGui::Checkbox((u8"리플릭"), &Config::Get().Silent);
				ImGui::Checkbox((u8"범위 그리기"), &Config::Get().FovDraw);
				ImGui::SliderFloat((u8"속도"), &Config::Get().AimSpeed, 1.5f, 0.00000f, "%.3f");
				ImGui::SliderFloat((u8"자동 발사 범위"), &Config::Get().m_TriggerScale, 200.f, 0.010f, "%.3f");
				ImGui::SliderFloat((u8"예측 범위"), &Config::Get().PreditLevel, 135.f, 0.00000f, "%.3f");
				ImGui::SliderFloat((u8"범위"), &Config::Get().Fov, 500.f, 0.f, "%.3f FOV");
				//ImGui::SliderFloat((u8"Y Axis"), &Config::Get().YPITCH, 1.0, 0.00000f, "%.2f");
				ImGui::Separator();

				ImGui::Text(u8"기타");
				ImGui::Checkbox((u8"자동 평타"), &Config::Get().자동평타);
				ImGui::Checkbox((u8"메르시 공힐버프 ( 휠버튼 )"), &Config::Get().Mecry);
				ImGui::Checkbox((u8"겐지 용검모드 ( 좌클릭 )"), &Config::Get().SAimbot);
				ImGui::Checkbox((u8"겐지 자동 질풍참 ( 우클릭 )"), &Config::Get().GENJISHIFT);
				ImGui::Checkbox((u8"아나 수면총 예측"), &Config::Get().AnaSkill);
				ImGui::Checkbox((u8"로드호그 그랩 예측"), &Config::Get().Roadhog);
				//ImGui::Checkbox((u8"Tracer Q Prediction"), &Config::Get().TrQSkill);
				ImGui::Checkbox((u8"시그마 E 예측"), &Config::Get().ESkill);
				ImGui::Checkbox((u8"둠피스트 로켓펀치 예측"), &Config::Get().DomPredit);
				ImGui::Separator();
			}
		}
		ImGui::End();
